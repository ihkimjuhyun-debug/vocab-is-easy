  ...
  const handleCheck = () => {
    ...
    for (const cleanTarget of targetOptions) {
      if (!isEnToKo) {
        ...
        const coreUser = cleanUser.replace(/(하다|되다|시키다|게하다|해지다|다)$/, '');
        const coreTarget = cleanTarget.replace(/(하다|되다|시키다|게하다|해지다|다)$/, '');

        const similarity = calculateSimilarity(cleanUser, cleanTarget);
        const threshold = cleanTarget.length > 8 ? 0.50 : 0.70;

        if (
          coreTarget === coreUser || 
          (coreTarget.includes(coreUser) && coreUser.length >= coreTarget.length * 0.5) ||
